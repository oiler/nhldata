...
We gave that null its best chance to fail. The fair objection is that pooling every strength state buries the signal: penalty kills are chaos, and changing teams changes the penalty kill around a goalie more than it changes the goalie. So we re-ran the test at 5v5 only, pre-trade measurements and post-trade results both, 59 cases, with the decision rules written down before we looked at anything. Nothing cleared there either. Stopping repeatability did tick up without the penalty-kill noise, from about 0.12 to about 0.17, which is still nothing you could rank with. A null that survives the cleanest cut of the data is a stronger null.

One skill is real: freezing the puck. Freeze rate repeats year over year (r ≈ 0.6–0.7), travels with goalies who switch teams, and is absent between teammates, so it is not a scorekeeper or team artifact. It held at the same strength when we cut to 5v5.

We priced it: each freeze suppresses about 0.02 expected goals over the next 30 seconds of play, mostly by killing rebound chances. A freeze on the penalty kill is worth more than double one at even strength. That fits — a frozen puck buys a tired unit a line change. Across a starter's season, the gap between a high-freeze and a low-freeze goalie is worth about two goals. Real, and small.
...
The site now presents goalies the way the evidence says it should: results next to the difficulty of what they faced, the one skill that repeats priced in goals, and each team's workload graded like an offensive line. Every goalie view also flips to a strict 5v5 cut, for readers who want the cleanest slice of the game. There is no goalie ranking, because the honest finding is that we cannot rank goalies from this data.
