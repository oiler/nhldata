...
## 6c. P4+P5 outcomes and P6 design (addendum, 2026-07-18; approved by oiler)

**P4+P5 outcomes (branch goalie-eval-p1, 8ff2881..af99be0, final-reviewed ready-to-merge):**

- **The `is_rebound` sign anomaly is era-dependent, not definitional.** The §6b mandatory item is resolved with a null: every candidate redefinition (post-save-only, windows 2–5 s) is negative on pooled data, and an era split shows the anomaly is post-2023 only (coefficients +0.38/+0.10 in 2021–22 vs −0.29/−0.33/−0.34 in 2023–25) with no dt-mass shift — so it is not the timestamp-window artifact that hit freeze. Definition unchanged; per-season fits already carry era-appropriate coefficients. Open mechanism question → P6 Task 1 (below). Documented in `docs/data-limitations.md`.
- **difficulty↔perf_z correlation (+0.18 all games, +0.23 full starts) is a sort-on-own-prediction artifact, not a calibration defect** (shot-level reliability near-diagonal; within-goalie residual r 0.191 vs between-goalie 0.048). Usage rule: per-game perf_z/gsax comparisons across difficulty bands are inflated by shared xGA noise; same-band and season-aggregate uses are sound; GA~xGA recalibration rejected (would curve-fit the artifact).
- **Mean per-game perf_z is −0.065, not 0, by pull-truncation selection** (pulled stints pair tiny variance with strongly negative GSAx; total GSAx ≈ 0 exactly; full-start subset +0.045). Property of the metric, not a bug.
- **Per-arena freeze-timing check: clean null** (max |offset| 0.021) — no arena adjustment needed; the §6b P4 item closes.
- Actual P5 filenames: `goalie_games_<season>.csv`, `wp_table.csv`, `game_difficulty.csv`, `game_ledger.csv`, `team_environment.csv`, `arena_freeze_offsets.csv` (supersedes §9's `leverage_ledger.csv` placeholder).

**P6 design decisions (this addendum governs where it differs from §6's sketch):**

- **Registry (supersedes §6's ≥1,000 default).** Measured reality: the ≥1,000-fenwick-each-side floor yields only 34 season-boundary cases over the four transition years, not the ~80–100 §2 estimated. Decision: floor lowered to ≥600 fenwick each side with each case weighted by min(pre, post) fenwick (n=60 season-boundary), **plus mid-season trades** as additional cases under the same rule (24 multi-team goalie-seasons, partially overlapping; expected usable total ≈ 60–75). Fallback if the mid-season path proves disproportionate: season-boundary-only (option B). Season-boundary switches compare consecutive-season primary teams (primary = most fenwick faced).
- **Outcome variable.** Post-switch GSAx per shot (goalie-blind xG minus goals, rink-adjusted via P2) over the full qualifying post-switch stint. One common outcome for all candidates; component-level portability (freeze→freeze) lives in the repeatability suite, not the gate.
- **Baseline.** Matched-horizon GSAx with empirical-Bayes shrinkage toward league mean by workload — the baseline sees the same pre-switch window as the candidate it is compared against. Single-prior-season raw GSAx is also computed and reported as the literature-comparable anchor (r ≈ 0.12 family), but the gate is scored against the EB baseline.
- **Gate statistic.** Weighted Pearson Δr = r(candidate, outcome) − r(baseline, outcome), paired bootstrap over switch cases (~10k resamples, pure numpy), 90% CI; Spearman alongside as robustness; incremental regression (outcome ~ baseline + candidate) as a supporting diagnostic, not the gate. A candidate beats GSAx if its Δr CI excludes zero. All candidates appear in one table; with five candidate families a single nominal CI exclusion is weak evidence and the report must say so.
- **Candidates (pre-registered, five, no additions after the harness first runs):** chained goal-layer (stopping) terms; freeze terms; rebound-control terms; season-aggregate mean perf_z from the P5 ledger; and exactly one composite — a workload-weighted ridge combination of the component families, fitted on non-switch goalie-seasons only, weights frozen before any switch case is scored.
- **Rebound era probe is P6 Task 1, with a pre-registered decision rule.** Probe: `froze`/`rebound_generated` distributions per era, conditional on shot features. Outcomes → actions fixed in advance: (a) coding shift at 2023 → era-normalize rebound/freeze inputs before candidacy; (b) era-stable distributions → rebound terms enter as-is with the documented caveat; (c) ambiguous → rebound terms run era-B-only as a sensitivity line, not a headline candidate.
- **Mid-season leakage rule.** Per-goalie season terms are fitted on full seasons, so a mid-season case's pre-switch term would see post-trade shots. For mid-season cases only, the term model is refit with that goalie's post-trade shots excluded (cached one-time refits; the shared goalie-blind xG model is not refit — one goalie's stint is negligible across ~112k shots).
