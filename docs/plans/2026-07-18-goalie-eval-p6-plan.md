...
- Mid-season candidate terms: for `midseason` cases, `term_lookup`'s chained values for the switch season are REPLACED by the cached refit terms (oriented the same way; layers in the `normalize` set pass through the season-population z-transform first — execution fix f928e99). `prior_terms` for a refit = chained terms from `goalie_terms_<season-1>.csv` per layer (empty dict for 2021). `rebound_control_indep` is set to NaN for midseason cases (the full-season term_indep would contain post-trade shots — execution fix f67a6ff).
- Pseudo-case outcome window (correction, final review 2026-07-18): `case_outcome` reused for pseudo-cases must restrict the post window to `first_post_season` — the registry's registered definition — not the full remaining stint. Original text's unqualified `case_outcome` reuse contradicted Task 2's interface; fixed in f67a6ff, conclusion-neutral (K and gate table unchanged).
