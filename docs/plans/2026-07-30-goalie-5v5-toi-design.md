...
1. **The data is already there.** `data/<season>/generated/timelines/csv/*.csv` carries `situationCode` alongside `awayGoalie` and `homeGoalie` on every second. This is the same source and the same method `compute_competition.py:67-75` already uses for skater 5v5 TOI.
...
`toi_5v5.py` writes an artifact rather than being called inline. Two modules call `load_toi` directly and a third stage consumes the result through `game_difficulty.csv`; computing it inline would walk 6560 timeline files once per consumer, duplicate the parsing in the places most likely to drift, and leave it untestable as a unit.
...
| `v2/browser/build_goalies_db.py:109` | Same swap, so `goalie_seasons.toi_s` is per-cut. |
...
| `v2/browser/app.py` glossary | Revise the GSAx/60 entry to drop the all-situations-only caveat; add xGA/60. Leave the `avg_toi_share` entry at `:186` alone — it describes a skater metric and is unaffected. |
...
**A goalie who played but saw no 5v5 ice** yields `toi_5v5_s` of 0, and rates are NaN through the existing `.where(toi > 0)` idiom already used by `events_per60`, `corsi_per60` and `gsax_per60`. This is a real zero, not a gap, and is reported separately from the NaN count.
...
Invariant test: for every goalie-season, 5v5 `toi_s` is strictly less than all-situations `toi_s`.
...
- No `(per 60 total TOI)` or `(all sit)` string remains in `v2/browser/pages/` — those labels exist only to disclose the mixed base this change removes. `app.py:186`'s "all situations" wording is a skater glossary entry and stays.
