...
- Tables:
  ...
  - `freeze_value` — zero or one row (per_freeze_xga_delta, window_s) written only if sub-project A's estimate is non-null; consumers treat absence as "no validated value."
